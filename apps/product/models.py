...
class ProductCategories(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.order:
            last_instance = ProductCategories.objects.filter(parent=self.parent).order_by('order').last()
...
class Products(models.Model):
    ...
    name = models.CharField(_('Название продукта'), max_length=512)
    code = models.IntegerField(default=0)
    article = models.CharField(_('Артикул'), max_length=512)
    product_size = models.CharField(_('Размер товара'), max_length=256, default="S-XXL")
    material = models.CharField(_('Материал'), max_length=512, default="S-XXL")
